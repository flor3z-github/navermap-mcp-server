...
import { getAddressTypeName } from '../utils/formatters.js';
...
export function registerReverseGeocodeTool(server: McpServer, apiClient: MapsApiClient): void {
  server.tool(
    ...
    async (args: ReverseGeocodeInput) => {
      try {
        const response = await apiClient.get<ReverseGeocodeResponse>(API_ENDPOINTS.REVERSE_GEOCODE, {
          coords: args.coords,
        ...
        const results = response.results.map((result) => ({
          유형: getAddressTypeName(result.name),
          코드: result.code.id,
